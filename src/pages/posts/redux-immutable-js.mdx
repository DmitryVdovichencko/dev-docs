...
Immutable.JS объекты, такие как `Map`, `List`, и т.п., могут иметь сложности с отладкой, поскольку при этом появлятся вся иерархия Immutable.JS-специфических свойств, до которой нам нет дела, тем временем данные, которые нам нужны лежат довольно глубоко.
...
## Стоит ли использование Immutable.JS всеъ этих усилий?
...
- Для селекторов используется последовательный интерфейс; больше не нужно следить что возвразаем Immutable.js объект или простой JavaScript объект.
...
### Используйте компоненты высшего порядка HOC для привеления Immutable.JS свойств Умных компонентов к JavaScript свойствам Тупых компонентов.
