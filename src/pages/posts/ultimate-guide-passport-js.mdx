...
## Сессии
Сессии и cookies, вообще-то довольно похожи, и можно легко запутаться, т.к. их беспрепятственно используют вместе. Главная разница между ними в том, где они хранятся.

Другими словами, Cookie уставнавливает сервер, но записываются и хранятся они в браузере. Если бы сервер захотел использовать этот Cookie для записи "состояния" пользователя, все закончилось бы сложной схемой постоянного наблюдения за cookie в браузере. А выглядело бы это как-то так:

Сервер: "Хей браузер, я только что авторизовал этого пользователя, запиши-ка cookie, чтобы я не забыл (Set-Cookie: user_auth=true; expires=Thu, 1-Jan-2020 20:00:00 GMT) в следующий раз когда ты что-нибудь попросишь"
Браузер: "Спасибо сервер! Я прикреплю этот Cookie к заголовкам запросов request header"
Браузер: "Хей сервер, могу я посмотреть на www.domain.com/protected? Вот cookie который ты мне отправлял в прошлый раз."
Сервер: "Конечно можешь. Вот эта страничка. И, кстати я к ней прикреплю еще Set-Cookie header (Set-Cookie: marketing_page_visit_count=1; user_ip=192.1.234.21), поскольку моя компания хочет знать сколько народу и с каких компов зашло на эту страничку: для маркетинга."
Браузер: Okay, добавлю этот cookie к заголовкам запросов request header
Браузер: Хей сервер, можешь отправить мне содержимое www.domain.com/protected/special-offer? Вот все cookies, что ты мне отпралял до сих пор.(Cookie: user_auth=true; expires=Thu, 1-Jan-2020 20:00:00 GMT; marketing_page_visit_count=1; user_ip=192.1.234.21)
Как вы уже поняли, чем больше страниц посещает браузер, тем больше cookies устанавливает сервер, и тем больше cookies браузер  будет прикреплять к каждому заголовку запроса request Header.

Сервер в свою очередь должен уметь распарсить все cookies и предпринять определенные действия при отсутствии или наоборот присутствии некоторых cookie. И тут возникает вопрос... Почему бы серверу не хранить всю эту информацию в базе данных и использовать специальный "session ID" для идентификации действий пользователя?

Для этого и нужны сессии. Как я заметил выше, главное различие cookie и сессии в том, где их хранят. Сессия хранится в Data Store (обычно база данных), тогда как Cookie - в браузере. Поскольку сессии харнятся на сервере, туда можно записывать конфиденциальную информацию. Хранить конфиденциальную инфу в cookie будет в высшей степени небезопасно.

Теперь немного усложним задачу, и поговорим о том как использовать cookies и сессии вместе.

Поскольку Cookies это способ обмена метаданными между клиентом и сервером (через HTTP Headers), сессия должна использовать cookies. Самый простой способ посмотреть как это работает - на примере приложения с авторизацией используя Node + Express + MongoDB. Я предполагаю что вы знакомы с базовыми принципами разработки приложений на Express, но постараюсь объяснять некоторые моменты по мере необходимости.

Начнем с создания базового приложения и установки зависимостей:
...
Вот как будет выглядеть `app.js`. Рекомендую пробежаться по комментам, прежде чем читать дальше.
 ...
 * -------------- ОБЩИЕ НАСТРОЙКИ ----------------
 */
// Получаем доступ к переменным окружения, указаным в .env файле с использованием синтакса `process.env.VARIABLE_NAME`
require("dotenv").config();
// Создаем приложение Express
var app = express();
// Миддлвары Express позволяющие парсить JSON и x-www-form-urlencoded в теле запроса
// Они такие же как и `bodyParser` - скорее всего вы видели здесь bodyParser во многих приложениях
 ...
 */
/**
 * Соединяемся с MongoDB Server используя параметр в `.env` файле. Не забудьте добавить строку
 * в `.env` файл:
 *
 * DB_STRING=mongodb://<user>:<password>@localhost:27017/database_name
 */
const connection = mongoose.createConnection(process.env.DB_STRING);
// Создаем простенькую схему User. hash и salt будут получены при регистрации юзера
const UserSchema = new mongoose.Schema({
  username: String,
  hash: String,
  salt: String,
});
// Определяем модель, которую будет использовать приложение
mongoose.model("User", UserSchema);
/**
 * -------------- НАСТРОЙКА СЕССИИ ----------------
 */
/**
 * MongoStore используется для хранения данных сессии. Подробнее об этом поговорим попозже.
 *
 * Заметьте `connection` для MongoStore тот же connection что мы использовали выше
 */
...
/**
 * Документация по всем настройкам для сессий - https://www.npmjs.com/package/express-session
 *
 * Краткий обзор того что здесь (подробнее - дальше):
 *
 * secret: Случайная строка, используемая для "авторизации" сессии. На проде,
 * неплохо было бы установить длинную рандомную строку
 *
 * resave: если установим true, то будем принудительно сохранять сессию, даже если ничего не менялось. Если его не установить,
 * получим предупреждение в терминале
 *
 * saveUninitialized: похож на resave, если поставить true, то будем принудительно сохранять сессию, 
 * даже если она не инициализировалась
 */
app.use(
  session({
 ...
 * -------------- РОУТЫ ----------------
 */
// Авторизация пользователя http://localhost:3000/login
app.get("/login", (req, res, next) => {
  res.send("<h1>Login Page</h1>");
});
app.post("/login", (req, res, next) => {});
// Регистрация пользователя http://localhost:3000/register
...
app.listen(3000);
```
Сначала разберемся как `express-session` работает в этом приложении. Это "миддлвара" ("middleware"), специальный термин для функциии, которая что-либо меняет в нашем приложении.

#### Краткий обзор по Express Middleware
Например, у нас есть следующий код:

```javascript
const express = require("express");
var app = express();
// Специальная мидллвара
function myMiddleware1(req, res, next) {
  req.newProperty = "my custom property";
  next();
}
// Еще одна специальная мидллвара
...
Как видите, это достаточно простое Express приложение в котором есть две миддлвары и один роут, на который можно перейти в браузере по адресу [http://localhost:3000](http://localhost:3000). Если вы запустили приложение и попробовали перейти по адресу, вы увидите "Custom Property Value: undefined", потому что просто объявить виддлвару недостаточно.

Мы должны объяснить приложению Express, что нужно использовать эти миддлвары. Мы можем сделать это несколькими способами. Сначала, сделаем это в роуте.
...
Если передать первую миддлвару в качестве аргумента для роута вы увидите как "Custom Property Value: my custom property" отобразится в браузере. Что здесь произошло:

- Приложение инициализировалось
- Пользователь зашел на http://localhost:3000/ в браузере, что спровоцировало вызов функции `app.get()`.
- Приложение Express сначала проверит есть ли какая-нибудь "глобальная" миддлвара для всех маршрутов, но ничего не найдет.
- Приложение Express посмотрит на функцию `app.get()` и заметит, что установлена миддлвара перед вызовом колбэка. Приложение запустит миддлвару и передаст ей объекты `req`, `res`, и колбэк `next()`.
Сначала, миддлвара `myMiddleware1` установит `req.newProperty`, затем вызовет функцию `next()`, которая скажет Express приложению: "Переходи к следующей миддлваре". Если миддлвара не вызовет `next()`, браузер застрянет и ничего не вернет.
Приложение Express не увидит других миддлвар, так что закончит с обработкой запроса и отправит результат.
Это один из способов использования миддлвары, и именно так работает функция `passport.authenticate()`.

Другой вариант использования миддлвары: объявить ее "глобально". Например:
```javascript
const express = require("express");
var app = express();
// Пользовательская миддлвара
function myMiddleware1(req, res, next) {
  req.newProperty = "my custom property";
  next();
}
// Другая пользовательская миддлвара
function myMiddleware2(req, res, next) {
  req.newProperty = "updated value";
  next();
}
...
app.get("/", myMiddleware1, (req, res, next) => {
  // Отправит "Custom Property Value: my custom property
  res.send(`<h1>Custom Property Value: ${req.newProperty}`);
});
...
С такой  структурой приложения, посетив [http://localhost:3000/](http://localhost:3000/), вы увидите, что возвращается то же значение, что и прежде. Это потому, что вызов `app.use(myMiddleware2)` произошел перед  обработчиком роута `app.get('/', myMiddleware1)`. Если мы уберем миддлвару из роута, то увидим обновленное значение в браузере.

```javascript
app.use(myMiddleware2);
app.get("/", (req, res, next) => {
  // Отправит "Custom Property Value: updated value
  res.send(`<h1>Custom Property Value: ${req.newProperty}`);
});
```

Такого же результата можно достичь поместив вторую миддлвару после первой в обработчике роута.
...
Это был краткий обзор того как работают миддлвары Express, он поможет нам разобраться, что происходит в миддлваре `express-session`.

### Принцип работы мидддлвары Express Session
Как я уже упоминал ранее, модуль `express-session` предоставляет нам миддлвару, которую мы можем использовать у себя в приложении. Объявляется она так:

```javascript
// И еще раз, ссылка на официальную доку - https://www.npmjs.com/package/express-session
app.use(
  session({
    secret: process.env.SECRET,
    resave: false,
...
Вот коротко то, чем занимается миддлвара Express Session:

Когда  вызывается роут, миддлвара проверяет есть ли данные сессиии  представленные в Session Store (база данных MongoDB в нашем случае, раз уж мы используем для хранения сессии `connect-mongo`).

Если данные сессии нашлись, миддлвара проверит их и сообщит браузеру можно ли использовать сессию. Если сессия прошла валидацию, то браузер прикрепит id сессии `connect.sid` в Cookie HTTP запроса. 

Если сессия не найдена, миддлвара создаст новую сессию, создаст зашифрованный хеш сессии и запишет значение `connect.sid` в Cookie. Затем это значение будет прикреплено в заголовке ответа Set-Cookie (Set-Cookie: connect.sid=зашифрованный хеш).
Возможны вы удивились, зачем все это вообще, и как это реально работает.

Если вы помните из кратого обзора по миддлварам Express, я говорил о том, что миддлвара имеет возможность  менять объекты `req` и `res`, которые передаются от миддлвары к миддлваре до тех пор, пока не дойдет до конца HTTP запроса. Также как мы устанавливали кастомные свойства на объект `req`, мы можем назначить кое- что поинтереснее, например объект сессии со своими свойствами и методами.

Именно это и делает миддлввара `express-session`. Когда создается новая сессия, следующие свойства добавляются к объекту `req`:

* `req.sessionID` - рандомно сгенерированный уникальный UUID. Вы можете объявить специальную функцию для генерации ID назначив параметр `genid`. Если вы этого не сделали, будет использоваться модуль `uid-safe`.
...
```
req.session - объект сессии. Здесь хранятся данные сессии, также можно устанавливать свои свойства. Например, вы хотите отследить сколько раз была загружена определенная страничка в рамках одной сессии:
...
`req.session.cookie` - объект Cookie. Здесь определяется поведение cookie которое записывает захешированный ID сессии в браузер. Помните, как только cookie установлен, браузер будет автоматически прикреплять его к каждому HTTP запросу, пока не истечет срок действия.


## Как работает стратегия Passport JS Local Strategy
